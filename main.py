...
def create_buttons():
    buttons_sprites = ['data/gfx/flap_indicator.png', 'data/gfx/speed_indicator.png', 'data/gfx/beanup_indicator.png']
    buttons = []
    # adding three buttons
    for i in range(3): buttons.append(UpgradeButton(3, buttons_sprites[i], ((250 + i * 125), 393)))
    buttons[-1].price = 30
    return buttons
...
def main():
    ...
    start_button = Button("Start", (DISPLAY.get_width() / 2, 292))
    # getting 5 beans
...
"""
        ...
        start_button.draw(DISPLAY, font_small)
    ...
    while True:
        ...
        for button in buttons:
            button.draw(DISPLAY, font_small, font_20)
        ...
        for bean in beans:
            ...
            if (checkCollisions(player.position.x, player.position.y, player.currentSprite.get_width(),
                                player.currentSprite.get_height(), bean.position.x, bean.position.y,
